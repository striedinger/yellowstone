...
const generateBorderClasses = border => {
  if (Array.isArray(border) && border.length > 0 && border.length <= 4) {
    const borderClasses = border.map(direction => {
      return spacing[`border-${direction}`];
    });
    return classnames(borderClasses);
  }
  return null;
};

export default ({ border, margin, padding }) => {
  return classnames(generateBorderClasses(border), generateMarginClasses(margin), generatePaddingClasses(padding));
};
